...
export function createApp(databasePath = 'data/pawpass.sqlite', options = {}) {
  ...
  app.post('/api/ai/chat', async (req, res) => {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({})); if (!response.ok) { console.error(`Gemini request failed (${response.status}) using ${model}:`, data.error?.status || data.error?.message || 'unknown error'); return res.status(502).json({ error: 'PawPass AI is temporarily unavailable. Check the server Gemini configuration.' }); }
      const raw = data.candidates?.[0]?.content?.parts?.[0]?.text?.replace(/^```json\s*|\s*```$/g, '').trim(); const answer = JSON.parse(raw || '{}'); if (typeof answer.reply !== 'string') throw new Error('Invalid AI response.');
      let requestId = null;
    ...
    } catch (error) { console.error('PawPass AI response error:', error instanceof Error ? error.message : 'unknown error'); res.status(502).json({ error: 'PawPass AI could not complete that message. Please try again.' }); }
  });
